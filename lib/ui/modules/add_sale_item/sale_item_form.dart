...
class SaleItemForm extends StatefulWidget {
  ...
  const SaleItemForm({Key key,@required  this.scaffoldKey,@required  this.formKey,@required  this.onSave,@required  this.submit,@required  this.defaultData,@required this.status,@required this.addPhoto, this.buttonText="Add", this.photos, this.deletePhoto}) : super(key: key);
  @override
  _SaleItemFormState createState() => _SaleItemFormState();
}
...
class _SaleItemFormState extends State<SaleItemForm> {
  ...
  void initState() {
      ...
      //   descriptionController.text=widget.defaultData['desc'];
            ...
            child: Column(
              ...
              children: <Widget>[
                ...
                FormInputField(
                  ...
                  nextFocusNode: focusNodeDescription,
                  textInputAction: TextInputAction.next,
                  validator: (value)=> Validator(context).isNotEmpty(value),
                  onSave: (value) {widget.onSave('title',value);},
                  outterBorder: true,
                  useLabel: true,
                ),
                FormInputField(
                  ...
                  onSave: (value) {
                    widget.onSave('description',value);
                  },
                  outterBorder: true,
                  useLabel: true,
                ),
                FormInputField(
                  ...
                  validator: (value){
                    if (value == null || value.length == 0) {
                      return "${AppLocalizations.of(context).translate("price")} ${AppLocalizations.of(context).translate("missing_data")}";
                    }else  if (double.tryParse(value)==null) {
                      return AppLocalizations.of(context).translate("value must be an integer");
                    }
                  },
                  onSave: (value) {widget.onSave('price',double.tryParse(value));},
                  outterBorder: true,
                  useLabel: true,
                ),
                widget.photos.length>0?Center(
                  child: Wrap(
                    children:widget.photos.map((photo) {
                      if(photo!=null)
                     return Badge(
                      badgeColor: Colors.red,
                      badgeContent: InkWell(onTap:()=> removePhoto(photo),child: Icon(FontAwesomeIcons.times)),
                ...
                InkWell(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      SizedBox(height: 10),
                      Text(AppLocalizations.of(context).translate("enter photos"),style: Theme.of(context).textTheme.bodyText1),
                      SizedBox(height: 30),
                    ],
                  ),
                ),
  ...
  Future<void> getPhotos() async {
      ...
      widget.addPhoto(file);
  }

  void removePhoto(photo) {
      widget.deletePhoto(photo);
  }
